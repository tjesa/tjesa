...
import FormWorkspaceClient from '@/components/tools/FormWorkspaceClient';
...
export default async function FormsPage() {
  ...
  let account = null;
  if (workspaceId) {
    account = await getAccount(`${workspaceId}_forms`);
  }

  const clientId = process.env.NOTION_FORMS_CLIENT_ID || process.env.NOTION_CLIENT_ID;
  const redirectUri = process.env.NOTION_FORMS_REDIRECT_URI || process.env.NOTION_REDIRECT_URI;
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxWidth: '480px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--sand-dim)', lineHeight: '1.6' }}>
              The Form Builder uses its own dedicated Notion integration.
              Connect it to grant access to the databases you want to create public forms for.
            </p>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%', maxWidth: '360px' }}>
            <a
              id="forms-connect-notion-btn"
              href={oauthUrl}
              style={{
                display: 'flex',
                alignItems: 'center',
  ...
  const configs = await getConfigs(`${workspaceId}_forms`);
  // Filter for form-specific configs
  const formConfigs = configs.filter(c => c.tool === 'form_builder');

  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {/* Ancient Egypt hieroglyphic watermark */}
      <div className="hieroglyph-bg" />
      <FormWorkspaceClient account={account} initialConfigs={formConfigs} oauthUrl={oauthUrl} />
    </main>
  );
}
